import { BASE_URL, SLASH, QUEUE, BASIC_INPUT, ADD_ELEMENT, DELETE_ELEMENT, DELETE_ALL, BASIC_CIRCLE, DEFAULT_COLOR, 
    CHANGING_COLOR } from "../../src/constants/const";
import { SHORT_DELAY_IN_MS } from "../../src/constants/delays";
  
describe("Проверка компонента Очередь", function() {
  ...
  it("добавление и удаление элемента в очередь", () => {
  ...
  it("удаление всех элементов из стека", () => {
    ...
    cy.get("@add").click();
    cy.get(BASIC_CIRCLE).eq(0).contains("AA");
    cy.wait(SHORT_DELAY_IN_MS);
    cy.get("@input").type("BB");
    cy.get("@add").click();
    cy.get(BASIC_CIRCLE).eq(1).contains("BB");
    cy.wait(SHORT_DELAY_IN_MS);
    cy.get("@input").type("CC");
    cy.get("@add").click();
    cy.get(BASIC_CIRCLE).eq(2).contains("CC");
