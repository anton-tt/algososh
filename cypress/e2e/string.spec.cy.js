...
describe("Проверка компонента Строка", function() {
  ...
  it("переворачивание строки c чётным числом элементов", () => {
  ...
  it("переворачивание строки c нечётным числом элементов", () => {
    cy.get("@input").type("кот");
    cy.get("@button").click().should("be.disabled");
    cy.get("@input").should("be.empty");
    ...
    cy.get("@firstElement").should("have.css", "border-color", CHANGING_COLOR).contains("к");
    cy.get("@secondElement").should("have.css", "border-color", DEFAULT_COLOR).contains("о");
    cy.get("@thirdElement").should("have.css", "border-color", CHANGING_COLOR).contains("т");

    cy.get("@firstElement").should("have.css", "border-color", MODIFIED_COLOR).contains("т");
    cy.get("@secondElement").should("have.css", "border-color", MODIFIED_COLOR).contains("о");
    cy.get("@thirdElement").should("have.css", "border-color", MODIFIED_COLOR).contains("к");
  });
